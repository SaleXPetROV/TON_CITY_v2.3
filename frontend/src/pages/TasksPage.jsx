...
const LOCAL_LABELS = {
  ...
  dailyActivityTitle: {
    en: 'Complete daily activity to claim', ru: 'Выполните дневную активность, чтобы забрать награду',
    es: 'Completa la actividad diaria para reclamar', zh: '完成每日活动即可领取',
    fr: 'Terminez l’activité quotidienne pour réclamer', de: 'Erledige die Tagesaktivität zum Abholen',
    ja: 'デイリー活動を完了して受け取る', ko: '일일 활동을 완료하고 수령', id: 'Selesaikan aktivitas harian untuk klaim',
  },
  dailyBuyLabel: {
    en: 'Buy resources', ru: 'Купить ресурсы', es: 'Comprar recursos', zh: '购买资源',
    fr: 'Acheter des ressources', de: 'Ressourcen kaufen', ja: '資源を購入', ko: '자원 구매', id: 'Beli sumber daya',
  },
  dailySellLabel: {
    en: 'Sell resources', ru: 'Продать ресурсы', es: 'Vender recursos', zh: '出售资源',
    fr: 'Vendre des ressources', de: 'Ressourcen verkaufen', ja: '資源を売却', ko: '자원 판매', id: 'Jual sumber daya',
  },
};
          ...
          {!expanded ? (
            ...
            <CardContent className="p-4" data-testid={`task-expanded-${task.id}`}>
              ...
              {(task.partner_need != null) && (() => {
                ...
                const pct = (ready && need > 0 && have != null)
                  ? Math.min(100, Math.round((have / need) * 100)) : 0;
                return (
                  <div className="mt-4 pt-4 border-t border-white/10" data-testid={`quest-progress-${task.id}`}>
                    ...
                    <div className="w-full h-2 rounded-full bg-white/10 overflow-hidden">
                      <div className="h-full bg-cyber-cyan transition-all" style={{ width: `${pct}%` }} data-testid={`quest-progress-bar-${task.id}`} />
                    </div>
                    {!ready && (
                      <p className="text-[10px] text-text-muted mt-1">{t('questProgressPending') || 'Обновляется…'}</p>
                    )}
                  </div>
                );
              })()}
              ...
              <Card className="glass-panel border-yellow-500/30 bg-gradient-to-br from-yellow-500/10 to-orange-500/5" data-testid="daily-login-card">
                <CardContent className="p-3 sm:p-4">
                  <div className="flex items-center justify-between gap-2 mb-2 sm:mb-3">
                    ...
                    <Button size="sm" disabled={daily.claimed_today || busy.__daily || !daily.activity_met} onClick={claimDaily}
                  ...
                  {/* Activity gate: must buy AND sell resources today */}
                  {!daily.claimed_today && (
                    <div className="mb-3 space-y-2" data-testid="daily-activity">
                      <div className="text-[11px] font-semibold text-yellow-300 uppercase tracking-wide">
                        {LOCAL_LABELS.dailyActivityTitle[lang] || LOCAL_LABELS.dailyActivityTitle.en}
                      </div>
                      {[
                        { key: 'buy', label: LOCAL_LABELS.dailyBuyLabel, have: daily.activity_bought || 0, need: daily.activity_buy_required || 0, color: 'bg-cyan-400' },
                        { key: 'sell', label: LOCAL_LABELS.dailySellLabel, have: daily.activity_sold || 0, need: daily.activity_sell_required || 0, color: 'bg-emerald-400' },
                      ].map((row) => {
                        const pct = row.need > 0 ? Math.min(100, Math.round((row.have / row.need) * 100)) : 100;
                        const done = row.have >= row.need;
                        return (
                          <div key={row.key} data-testid={`daily-activity-${row.key}`}>
                            <div className="flex items-center justify-between text-[11px] mb-0.5">
                              <span className="text-text-muted flex items-center gap-1">
                                {done && <CheckCircle2 className="w-3 h-3 text-emerald-400" />}
                                {row.label[lang] || row.label.en}
                              </span>
                              <span className={`font-mono font-bold ${done ? 'text-emerald-400' : 'text-white'}`}>
                                {row.have}/{row.need}
                              </span>
                            </div>
