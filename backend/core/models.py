...
class User(BaseModel):
    ...
    # ------------------------------------------------------------------ #
    # DRIFT-TOLERANT COERCION
    #
    # Real accounts accumulated over time can hold slightly "off" values in
    # Mongo (a numeric field stored as None or a string, or plots_owned /
    # businesses_owned holding dict objects instead of plain id strings).
    # A strict Pydantic model would raise ValidationError → get_current_user
    # returns HTTP 500 → the frontend treats 5xx as transient and RETRIES
    # FOREVER, which is exactly the endless "99%" preloader that only hits
    # SOME accounts (the ones whose data drifted). These validators coerce
    # such values back to their expected type so EVERY account loads.
    # ------------------------------------------------------------------ #
    @field_validator("xp", mode="before")
    @classmethod
    def _coerce_xp(cls, v):
        try:
            return int(float(v)) if v is not None and v != "" else 0
        except (ValueError, TypeError):
            return 0

    @field_validator("balance_ton", "total_turnover", "total_income", mode="before")
    @classmethod
    ...
    @field_validator("plots_owned", "businesses_owned", mode="before")
    @classmethod
    def _coerce_id_list(cls, v):
        ...
        for item in v:
            ...
            if isinstance(item, str):
                ...
                # Extract a sensible id from an object-shaped entry.
                _id = item.get("id") or item.get("_id") or item.get("business_id") or item.get("plot_id")
                if _id is not None:
                    out.append(str(_id))
            else:
                out.append(str(item))
...
class WithdrawRequest(BaseModel):
    ...
    # Telegram Mini App biometric confirmation token — alternative to TOTP
    # for users who confirmed via Face ID / fingerprint inside the Mini App.
