"""Prepare a clean level-0 test state for the frontend (UI) test run.

Usage: python tests/_iter5_zero_ui_prep.py           -> reset both users, print 2 free empty cells
       python tests/_iter5_zero_ui_prep.py inspect   -> print current state only
"""
import sys
...
EMAILS = ["testuser@example.com", "sanyanazarov212@gmail.com"]
...
def inspect():
    for e in EMAILS:
        u, uid = ids(e)
        print(e, "bal", u.get("balance_ton"), "bonus", u.get("bonus_balance"),
              "grad", u.get("has_graduated_zero"),
              "biz", list(db.businesses.find({"owner": {"$in": uid}}, {"_id": 0, "id": 1, "level": 1, "x": 1, "y": 1, "is_zero_business": 1})),
              "lots", list(db.land_listings.find({"seller_id": {"$in": uid}}, {"_id": 0, "id": 1, "price": 1, "status": 1})))
...
if __name__ == "__main__":
    if len(sys.argv) > 1 and sys.argv[1] == "inspect":
        ...
        reset("testuser@example.com", 100.0)
        reset("sanyanazarov212@gmail.com", 1230.0)
        print("FREE_CELLS", free_cells(3))
        inspect()
