...
def create_tasks_router(db, get_current_user, get_admin_user):
    ...
    async def _do_partner_verify(user_doc: dict, task: dict) -> dict:
        ...
        # засчитываются автоматически. Если поле/порог не заданы — HTTP 200 == успех.
        ...
        if check_field and check_min is not None:
            ...
            baseline = await _ensure_partner_baseline(user_doc, task, current=current)
            if current is None or baseline is None:
                raise HTTPException(status_code=503, detail=_msg(lang, "quest_reward_unavailable"))
            have = max(0.0, float(current) - float(baseline))
            need = float(check_min)
            if have < need:
                logger.info(f"partner quest {task.get('id')} below threshold for {uid}: прирост {have} < {need}")
                raise HTTPException(status_code=400, detail=_msg(lang, "partner_progress",
                                                                 have=_fmt_num(have), need=_fmt_num(need)))
        return {"ok": True}
    ...
    async def _partner_metric_value(user_doc: dict, task: dict, max_age: int = 30):
        ...
        if status != 200:
            return None
        val = _extract_check_value(body, check_field)
        if val is not None:
    ...
    async def _partner_progress(user_doc: dict, task: dict):
        ...
        have — прирост метрики с момента выдачи задания (current - baseline)."""
        check_min = task.get("partner_check_min")
